...
Mặc định **không chép**. `binh-luan.js` có lời hứa ở đầu file rằng địa chỉ
người đọc không rời cơ sở dữ liệu, và bản sao lưu này giữ đúng lời hứa đó — cột
`email` bị gỡ **ngay trong Worker**, trước khi gói rời máy chủ.

Đánh đổi: khôi phục từ bản sao lưu thì **mất phần liên hệ**. Bình luận còn
nguyên, chỉ không biết ai gửi nếu cần trả lời riêng.

Muốn chép đầy đủ thì sửa `wrangler.jsonc`:

```jsonc
"SAO_LUU_EMAIL": "1"
```

Cân nhắc trước khi bật: địa chỉ người đọc sẽ nằm trong Google Drive, và ở lại
đó trong mọi bản sao lưu của Drive.
