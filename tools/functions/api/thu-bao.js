...
/* ══════════════════════════════════════════════════════════════════════
   ...
   ── EMAIL NGƯỜI BÌNH LUẬN: MỘT CÔNG TẮC, KHÔNG PHẢI MỘT LUẬT ──────────
   `SAO_LUU_EMAIL` quyết định cột `email` có đi theo gói hay không.

   Chỗ dễ hiểu nhầm: lời hứa ở đầu `binh-luan.js` nói email **không rời cơ sở
   dữ liệu** — nghĩa là không đi qua API xuống trình duyệt người đọc, và không
   vào thư báo. Nó KHÔNG có nghĩa là không lưu: cột ấy nằm nguyên trong bảng
   `binh_luan` từ lúc người ta bấm Gửi, và chủ trang tra được bất cứ lúc nào
   bằng Console của D1.

   Nên công tắc này chỉ quyết định đúng một chuyện: có chép THÊM một bản sang
   Google Drive hay không. Tắt thì vẫn liên hệ lại được hằng ngày qua D1; chỉ
   mất phần ấy trong đúng một tình huống — D1 mất sạch và phải dựng lại từ
   Sheet.

   Đang BẬT, và đó là lựa chọn của chủ trang chứ không phải mặc định của mã:
   một bản sao lưu mà khôi phục xong vẫn thiếu một cột thì chưa phải bản sao
   lưu đầy đủ. Cái giá đi kèm ghi ở chú thích trong wrangler.jsonc.

   Cột được gỡ ngay Ở ĐÂY khi tắt, trước khi gói rời Worker — lọc ở phía Apps
